...
//!
//! Generates the `raves_metadata_types::iptc` module!
//!
//! ```cargo
//! [package]
//! edition = "2024"
//!
//! [dependencies]
//! yaml-rust2 = { version = "0.10.3", default-features = true }
//! ```
...
impl RustTypes<'_> {
    fn output_to_file(self) {
        let out_dir = "../src/";
        let dest = Path::new(&out_dir).join("iptc.rs");

        // if it already exists, remove it!
        //
        // otherwise, it'll append to the end lol
        if let Ok(true) = fs::exists(&dest) {
            fs::remove_file(&dest).expect("remove existing iptc.rs to re-write it");
        }
